//
//  PinterestSegmentedControl.swift
//
//
//  Created by Bahdan Piatrouski on 28.01.24.
//
...
open class PinterestSegmentedControl: UIControl {
    ...
    private func reloadData(animated: Bool = true, sendAction: Bool = true) {
        ...
        if titleElements.contains(where: { $0.normalImage != nil }) || titleElements.contains(where: { $0.selectedImage != nil }) {
            titleH += style.titlePendingVertical
        }
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(PinterestSegmentedControl.handleTapGesture(_:)))
